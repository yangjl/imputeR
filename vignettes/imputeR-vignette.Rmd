...
probs <- error_mx(hom.error=0.02, het.error=0.1, imiss=0.2)
# simulate a GBS.array object
GBS.array <- sim.array(size.array=50, numloci=1000, imiss=0.2, selfing=0.5, het.error=0.1)
# set up all possiable halpotypes
win_length =10
haps <- setup_haps(win_length) 
chunklist <- phase_chunk(GBS.array, win_length, haps, verbose=TRUE)
if(length(chunklist) > 1){
    outchunks <- join_chunks(GBS.array, chunklist, join_length, verbose=TRUE)
}else{
    outchunks <- chunklist
}
# write as a data.frame
df <- write_phase(outchunks)
