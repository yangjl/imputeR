...
PhaseParent <- function(GBS.array, win_length=10, join_len=10, hom.error=0.02, het.error=0.8, imiss=0.2,
    ...
    if(length(haplist) > 1){
        out <- join_chunks(haplist, mom_array, progeny, ped, probs, verbose, unphased_mom, join_len)
...
#' haplist <- phase_chuck(GBS.array, haps, verbose, win_length)
#' 
join_chunks <- function(haplist, mom_array, progeny, ped, probs, verbose, unphased_mom, join_len){
    ...
    for(chunki in 2:length(haplist)){
        if(verbose){ message(sprintf("###>>> join chunks [ %s and %s, total:%s] ...", 
                                     chunki-1, chunki, length(haplist))) }
