...
#' 
#' 
PhaseParent <- function(GBS.array, win_length=10, join_len=10, hom.error=0.02, het.error=0.8, 
    ...
    probs <- gen_error_mat(hom.error, het.error)
...
sum_max_log_1hap <- function(gbsarray, winidx, dad_hap){
    ...
    maxlog2 <- lapply(1:nrow(ped1), function(x) {
        max_log_1hap_1kid(dad_hap, mom_haps, kid_geno=kgeno[[x]][winidx])
    }
...
max_log_1hap_outcrossed_kid <- function(dad_hap, mom_haps, kid_geno){
    ...
    geno_probs <- lapply(1:length(allgenos), function(geno){
        ...
        sum( unlist(lapply(1:length(dad_hap), function(zz) {
            ...
            tem <- probs[[2]][[ 1+tem_mom_haps[[geno]][zz] ]][allgenos[[geno]][zz]+1, kid_geno[zz]+1]
